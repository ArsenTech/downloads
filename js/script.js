...
const downloadBtns=document.querySelectorAll(".card-btn"),list = document.querySelectorAll(".list");
list.forEach((el) =>el.addEventListener("click", ()=>{
  for(let i=0; i<list.length; i++) list[i].classList.remove("active");
  el.classList.add("active");
  const cards = document.querySelectorAll(".card-container .grid-item"),dataFilter = el.dataset.filter;
  cards.forEach(card => {
    card.classList.remove("active");card.classList.add("hide");
    if (card.dataset.item===dataFilter||dataFilter==="all") {
      card.classList.remove("hide");card.classList.add("active");
    }
  });
}));
...
frmSearch.addEventListener("submit", e=>{
  ...
  const cards = document.querySelectorAll(".card-container .grid-item");
  cards.forEach((el,i)=>{

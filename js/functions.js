...
const displayCards = (cards,cardList) =>cards.forEach((card,i)=>{const el = document.createElement("div");
     ...
     </select><button class="card-btn icon" disabled aria-label="Download"><img src="files/icons/download.svg" alt="" width="30" height="30" loading="lazy"></button>` : `<button class="card-btn"${card.pathName ? ` data-file-path="${card.pathName}${card.isRelease ? "-release" : ""}"` : ""} data-type="${card.type}"><img src="files/icons/${card.type==="external" ? "launch" : "download"}.svg" alt="" width="30" height="30" loading="lazy">Download</button>`;
...
function downloadFile(e,o,s){if("external"===s)return void(location.href=e);const n=e.endsWith("-release"),l=e.replace("-release",""),d=document.createElement("a");d.href=n?`https://github.com/ArsenTech/downloads-files/releases/download/${l}`:`https://github.com/ArsenTech/downloads-files/raw/refs/heads/main/${l}`,d.download=l,d.click(),d.remove(),o.src="files/icons/download-done.svg",setTimeout((()=>{o.src="files/icons/download.svg"}),2e3)}
...
function changeSize(size, img){const filePath = `https://raw.githubusercontent.com/ArsenTech/wallpapers/refs/heads/main/coding${img}/${size.replace("x", " x ")}.png`;getWallpaperInfo(filePath);}

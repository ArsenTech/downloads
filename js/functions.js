...
function downloadFile(e,o,s){if("external"===s)return;const n=e.endsWith("-release"),l=e.replace("-release",""),d=document.createElement("a");d.href=n?`https://github.com/ArsenTech/downloads/releases/download/${l}`:`https://raw.githubusercontent.com/ArsenTech/downloads/refs/heads/static-images/${l}`,d.download=l,d.click(),d.remove(),o.src="images/icons/download-done.svg",setTimeout((()=>{o.src="images/icons/download.svg"}),2e3)}
async function getWallpaperInfo(e){const t=document.querySelector("#fileSize"),n=await fetch(encodeURI(e),{method:"HEAD"}),o=parseInt(n.headers.get("Content-Length"));t.innerHTML=formatBytes(o||0)}function changeWallpaper(img){const image = `images/wallpaperImg/coding${img}.webp`,preview = document.getElementById("preview-img");preview.src = image;return img;}
